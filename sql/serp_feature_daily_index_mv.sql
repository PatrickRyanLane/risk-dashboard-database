create materialized view if not exists serp_feature_daily_index_mv as
select sfi.date,
       sfi.entity_type,
       sfi.feature_type,
...
create unique index if not exists serp_feature_daily_index_mv_unique_idx
    on serp_feature_daily_index_mv (date, entity_type, feature_type);

create index if not exists serp_feature_daily_index_mv_date_idx
    on serp_feature_daily_index_mv (date);
