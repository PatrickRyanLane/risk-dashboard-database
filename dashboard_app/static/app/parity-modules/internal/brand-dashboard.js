...
function runInlineScript(scope) {
  ...
  const LOOKBACK_OPTIONS = [30, 45, 60, 90, 180, 365];
  ...
  const SERP_FEATURES_CRISIS_KEYS_PATH = () => `/api/v1/serp_features?entity=brand&days=${CRISIS_DAYS}&feature_type=top_stories_items`;
  ...
  const NARRATIVE_TAGS_PATH = d => `/api/v1/narrative_tags?entity=brand&date=${encodeURIComponent(d)}`;
  const NARRATIVE_TIMELINE_DAYS = 90;
  const NARRATIVE_TIMELINE_PATH = (d, company, days = NARRATIVE_TIMELINE_DAYS) =>
    `/api/v1/narrative_timeline?entity=brand&date=${encodeURIComponent(d)}&entity_name=${encodeURIComponent(company)}&days=${days}`;
  const NARRATIVE_OVERLAY_LIMIT = 0;
  const COMPOSITE_TAG_OVERLAY_STYLE = 'top-band';
  const NARRATIVE_OVERLAY_PATH = (company, startDate, endDate, limit = NARRATIVE_OVERLAY_LIMIT) =>
    `/api/v1/narrative_overlay?entity=brand&entity_name=${encodeURIComponent(company)}&start_date=${encodeURIComponent(startDate)}&end_date=${encodeURIComponent(endDate)}&limit=${limit}`;
  ...
  const _narrativeOverlayCache = new Map();
  const _narrativeOverlayPending = new Map();
  const PAGE_SIZE = 25;
  ...
  function getActiveAdvancedFilterCount(){
    ...
    if (sectorFilter) count += 1;
    if ((companySizeFilter || 'all') !== 'all') count += 1;
    if (crisisOnly) count += 1;
    if (nonCrisisOnly) count += 1;
    return count;
  }
  ...
  function updateAdvancedFilterBadge(){
    ...
    badge.dataset.visible = activeCount > 0 ? 'true' : 'false';
    btn.classList.toggle('active', activeCount > 0);
    btn.setAttribute('aria-label', activeCount > 0 ? `Filters (${activeCount} active)` : 'Filters');
  }

  function setFiltersDrawerOpen(next){
    ...
    filtersDrawerOpen = !!next;
    drawer.classList.toggle('open', filtersDrawerOpen);
    drawer.setAttribute('aria-hidden', filtersDrawerOpen ? 'false' : 'true');
    toggleBtn.setAttribute('aria-expanded', filtersDrawerOpen ? 'true' : 'false');
    backdrop.hidden = !filtersDrawerOpen;
    backdrop.classList.toggle('open', filtersDrawerOpen);
  }
  ...
  function bindFiltersDrawer(){
    ...
    const backdrop = document.getElementById('filtersDrawerBackdrop');
    const drawer = document.getElementById('filtersDrawer');
    if (toggleBtn && toggleBtn.dataset.boundFiltersToggle !== '1'){
      toggleBtn.dataset.boundFiltersToggle = '1';
      toggleBtn.addEventListener('click', () => {
        setFiltersDrawerOpen(!filtersDrawerOpen);
      });
    }
    if (closeBtn && closeBtn.dataset.boundFiltersClose !== '1'){
      closeBtn.dataset.boundFiltersClose = '1';
      closeBtn.addEventListener('click', () => setFiltersDrawerOpen(false));
    }
    if (backdrop && backdrop.dataset.boundFiltersBackdrop !== '1'){
      backdrop.dataset.boundFiltersBackdrop = '1';
      backdrop.addEventListener('click', () => setFiltersDrawerOpen(false));
    }
    if (drawer && drawer.dataset.boundFiltersEscape !== '1'){
      drawer.dataset.boundFiltersEscape = '1';
      document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && filtersDrawerOpen) setFiltersDrawerOpen(false);
      });
    }
  ...
  async function reloadLookbackData(){
    ...
    if (selectedCompany) await loadSerpFeaturesForEntity(selectedCompany);
    await renderAll();
  }
  ...
  async function selectCompany(name){
    ...
    await prefetchCompositeNarrativeOverlay(selectedCompany, getVisibleCompositeDateWindow());
    renderTable();
  ...
  async function loadRosterFavorites(){
    try{
      ...
      rows.forEach(r=>{
        ...
        if (sector) companySector.set(company, sector);
  ...
  function matchesCompanySize(company){
    const flags = fortuneFlags.get(company) || {};
    if (companySizeFilter === 'favorites') return favoriteCompanies.has(company);
  ...
  function hookDatePager(allDates){
    const goPrev = async () => {
      ...
      if (selectedCompany){
        await prefetchCompositeNarrativeOverlay(selectedCompany, sliceWindow(allDates, dateWindowStart, DATE_WINDOW_SIZE));
      }
      renderCharts();
    };
    const goNext = async () => {
  ...
  async function fetchCsvAny(cands){
    ...
    if (sharedData?.fetchCsvAny) {
      return sharedData.fetchCsvAny(cands);
    }
  ...
  function formatNarrativeTag(tag, group){
    const t = String(tag || '').trim();
    if (!t) return '';
    const g = String(group || '').trim().toLowerCase();
    if (g === 'non_crisis') return `${t} (non-crisis)`;
    return t;
  }

  async function getNarrativesForDate(d){
    if (_narrativeCache.has(d)) return _narrativeCache.get(d);
    const map = new Map();
    if (!d) return map;
    try{
      const rows = await fetchCsv(NARRATIVE_TAGS_PATH(d));
      (rows || []).forEach(r=>{
        const company = String(r.entity_name || r.company || '').trim();
        if (!company) return;
        const key = normEntityKey(company);
        if (!key) return;
        const primaryTag = String(r.primary_tag || '').trim();
        const primaryGroup = String(r.primary_group || '').trim();
        const displayTag = String(r.primary_display_tag || '').trim() || formatNarrativeTag(primaryTag, primaryGroup);
        const tags = Array.isArray(r.tags) ? r.tags : [];
        const displayTags = Array.isArray(r.display_tags) ? r.display_tags : tags.map(t => formatNarrativeTag(t, ''));
        map.set(key, {
          primary_tag: primaryTag || null,
          primary_group: primaryGroup || null,
          primary_display_tag: displayTag || null,
          tags,
          display_tags: displayTags,
          has_non_crisis: !!r.has_non_crisis,
          has_crisis: !!r.has_crisis,
        });
      });
    }catch{}
    _narrativeCache.set(d, map);
    return map;
  }

  function getVisibleCompositeDateWindow(){
    const {dates} = getDateSeries();
    const start = clampDateStart(dates.length);
    return sliceWindow(dates, start, DATE_WINDOW_SIZE);
  }

  function getCompositeNarrativeOverlayKey(entityName, visibleDates){
    const key = normEntityKey(entityName);
    const startDate = String(visibleDates?.[0] || '').trim();
    const endDate = String(visibleDates?.[visibleDates.length - 1] || '').trim();
    if (!key || !isISODate(startDate) || !isISODate(endDate)) return '';
    return `${key}|${startDate}|${endDate}|${NARRATIVE_OVERLAY_LIMIT}`;
  }

  function fitCompositeOverlayLabel(ctx, label, maxWidth){
    const raw = String(label || '').trim();
    if (!raw || !Number.isFinite(maxWidth) || maxWidth <= 8) return '';
  ...
  function layoutCompositeNarrativeWindows(windows){
    const sorted = [...windows].sort((a, b) =>
      String(a.start_date || '').localeCompare(String(b.start_date || '')) ||
      String(a.end_date || '').localeCompare(String(b.end_date || '')) ||
      String(a.display_tag || '').localeCompare(String(b.display_tag || ''), undefined, { sensitivity: 'base' })
    );
    const laneEnds = [];
    const laidOut = [];
    sorted.forEach(win => {
      let lane = laneEnds.findIndex(endDate => String(win.start_date || '') > String(endDate || ''));
      if (lane === -1){
        lane = laneEnds.length;
        laneEnds.push('');
      }
      laneEnds[lane] = String(win.end_date || '');
      laidOut.push({ ...win, lane });
    });
    return laidOut;
  }

  function getCompositeNarrativeOverlayLaneCount(windows){
    return windows.reduce((maxLane, win) => Math.max(maxLane, Number(win?.lane) || 0), -1) + 1;
  }

  function getCompositeNarrativeOverlayReservedHeight(windows, style = COMPOSITE_TAG_OVERLAY_STYLE){
    const laneCount = getCompositeNarrativeOverlayLaneCount(windows);
    if (!laneCount) return 0;
    const laneHeight = 16;
    const laneGap = 4;
    return 12 + (laneCount * laneHeight) + ((laneCount - 1) * laneGap);
  }

  function getCompositeNarrativeOverlayHit(chart, event){
    const regions = Array.isArray(chart?.$compositeNarrativeOverlayRegions) ? chart.$compositeNarrativeOverlayRegions : [];
    if (!regions.length) return null;
    const x = typeof event?.x === 'number' ? event.x : event?.native?.offsetX;
    const y = typeof event?.y === 'number' ? event.y : event?.native?.offsetY;
    if (!Number.isFinite(x) || !Number.isFinite(y)) return null;
    return regions.find(region => x >= region.left && x <= region.right && y >= region.top && y <= region.bottom) || null;
  }

  function getCompositeNarrativeOverlay(entityName, visibleDates){
    const key = getCompositeNarrativeOverlayKey(entityName, visibleDates);
    return key ? (_narrativeOverlayCache.get(key) || []) : [];
  }

  async function prefetchCompositeNarrativeOverlay(entityName, visibleDates){
    const key = getCompositeNarrativeOverlayKey(entityName, visibleDates);
    if (!key) return [];
    if (_narrativeOverlayCache.has(key)) return _narrativeOverlayCache.get(key);
    if (_narrativeOverlayPending.has(key)) return _narrativeOverlayPending.get(key);
    const startDate = String(visibleDates[0] || '').trim();
    const endDate = String(visibleDates[visibleDates.length - 1] || '').trim();
    const promise = (async () => {
      try{
        const payload = await fetchCsv(NARRATIVE_OVERLAY_PATH(entityName, startDate, endDate));
        const rawWindows = Array.isArray(payload?.windows) ? payload.windows : [];
        const windows = rawWindows
          .map(row => {
            const windowStart = String(row.start_date || '').trim();
            const windowEnd = String(row.end_date || '').trim();
            if (!isISODate(windowStart) || !isISODate(windowEnd) || windowStart > windowEnd) return null;
            if (windowStart > endDate || windowEnd < startDate) return null;
            return {
              tag: String(row.tag || '').trim(),
              display_tag: String(row.display_tag || row.tag || '').trim() || 'Crisis tag',
              group: String(row.group || '').trim().toLowerCase() || 'crisis',
              is_crisis: !!row.is_crisis,
              is_non_crisis: !!row.is_non_crisis,
              start_date: windowStart,
              end_date: windowEnd,
              duration_days: Number(row.duration_days || 0) || 0,
              negative_item_count: Number(row.negative_item_count || 0) || 0,
              active_on_end_date: !!row.active_on_end_date,
            };
          })
          .filter(Boolean);
        const laidOut = layoutCompositeNarrativeWindows(windows);
        _narrativeOverlayCache.set(key, laidOut);
        return laidOut;
      }catch{
        _narrativeOverlayCache.set(key, []);
        return [];
      } finally {
        _narrativeOverlayPending.delete(key);
      }
    })();
    _narrativeOverlayPending.set(key, promise);
    return promise;
  }
  ...
  async function init(){
    ...
    _narrativeOverlayCache.clear();
    _narrativeOverlayPending.clear();
    updateChartSkeletons();
    const sel = document.getElementById('dateSelect');
    ...
    if (sectorFilterSelect){
      sectorFilterSelect.onchange = ()=>{ currentPage = 1; updateAdvancedFilterBadge(); renderAll(); };
    }
    ...
    const nonCrisisBtn = document.getElementById('nonCrisisBtn');
    if (nonCrisisBtn){
      nonCrisisBtn.onclick = ()=>{
        nonCrisisOnly = !nonCrisisOnly;
        nonCrisisBtn.classList.toggle('active', nonCrisisOnly);
        updateAdvancedFilterBadge();
        renderAll();
      };
    }
    ...
    if (!window.__riskdashBrandStockModalBound){
      window.addEventListener('click', (event) => {
        if (event.target === document.getElementById('stockChartModal')) {
          closeStockChart();
        }
      });
      window.__riskdashBrandStockModalBound = true;
    }
  }
  ...
  async function buildRowsForDate(d){
    ...
    const narratives = await getNarrativesForDate(d);
    ...
    for (const [company, a] of articles.entries()){
      ...
      const narrative = narratives.get(normEntityKey(company)) || null;
      const narrativeTag = narrative?.primary_display_tag || null;
      ...
      out.push({
        ...
        has_non_crisis_narrative: !!(narrative && narrative.has_non_crisis),
        has_crisis_narrative: !!(narrative && narrative.has_crisis),
        narrative_tags: narrative?.display_tags || [],
      });
    }
    return out;
  }
  ...
  async function renderAll(){
    ...
    const rows = (await buildRowsForDate(d)).filter(r => {
      ...
      if (!matchesCompanySize(r.company)) return false;
      if (crisisOnly && !crisisEntityKeys.has(normEntityKey(r.company))) return false;
      if (nonCrisisOnly && !r.has_non_crisis_narrative) return false;
      return true;
    });
    filteredRows = rows;
    currentPage = 1;
  ...
  function renderTable(){
    ...
    if (currentSort.key){
      rows.sort((a,b)=>{
        ...
        if (typeof ka === 'string' || typeof kb === 'string') {
          return currentSort.dir * String(ka ?? '').localeCompare(String(kb ?? ''));
        }
    ...
    const rowsHTML = pageRows.map(r=>{
      ...
      const favStar = `<button class="fav-inline ${favActive ? 'active' : ''}" data-company="${esc(r.company)}" title="Toggle favorite">★</button>`;
      const narrativeTag = String(r.narrative_tag || '').trim();
      const narrativeIsNonCrisis = /\(non-crisis\)$/i.test(narrativeTag);
      const narrativeButtonClass = narrativeTag
        ? `${narrativeIsNonCrisis ? 'low' : 'high'}`
        : 'na';
      const narrativeButtonLabel = narrativeTag || 'N/A';
      const narrativeButtonTitle = (r.narrative_tags || []).join(', ') || 'View narrative timeline';
      const narrativeHtml = `<button type="button" class="pill narrative-pill-btn ${narrativeButtonClass}" data-action="narrative-timeline" data-company="${esc(r.company)}" title="${esc(narrativeButtonTitle)}">${esc(narrativeButtonLabel)}</button>`;
      ...
      return `<tr data-company="${esc(r.company)}">
        ...
        <td>${narrativeHtml}</td>
        <td class="action-col"><button class="coverageBtn" data-company="${esc(r.company)}" onclick="event.stopPropagation()">View</button></td>
      </tr>`;
    }).join('');
    ...
    tbody.querySelectorAll('.fav-inline[data-company]').forEach(btn=>{
      btn.onclick = async (e)=>{
        ...
        if (ok) {
          ...
          if (companySizeFilter === 'favorites' && !next) {
            renderAll();
          }
        }
        btn.disabled = false;
    ...
    tbody.querySelectorAll('button[data-action="narrative-timeline"]').forEach(btn=>{
      btn.onclick = (e)=>{
        e.stopPropagation();
        const company = String(btn.getAttribute('data-company') || '').trim();
        if (!company) return;
        showNarrativeTimeline({ company });
      };
    });
  ...
  function renderCharts(){
    ...
    const openChartDetailAt = (kind, date) => {
      ...
      if (!company) {
        openModal('Coverage Detail', '<div class="muted">Select a company row first, then click a chart data point.</div>');
        return;
      }
      if (kind === 'news') return void showHeadlines({ company }, date);
      if (kind === 'serp') return void showSerp({ company }, date);
      if (kind === 'features') return void showSerpFeatures({ company }, date);
    };
    ...
    const commonOpts = {
      ...
      scales:{
        ...
        y:{ ticks:{color:'#ebf2f2', stepSize:20, callback:v=>pct(v)},
            grid:{color:'transparent'}, min:0, max:100 }
      },
      plugins:{
    ...
    const compositeNarrativeWindows = getCompositeNarrativeOverlay(selectedCompany, d);
    const compositeNarrativeReservedHeight = getCompositeNarrativeOverlayReservedHeight(compositeNarrativeWindows);
    const compositeCardEl = document.querySelector('.chart-card[data-chart="composite"]');
    if (compositeCardEl) {
      compositeCardEl.style.minHeight = `${Math.max(460, 300 + compositeNarrativeReservedHeight)}px`;
    }
    const compositeNarrativeOverlayPlugin = {
      id: 'compositeNarrativeOverlay',
      afterDatasetsDraw(chart, _args, pluginOpts){
        const windows = Array.isArray(pluginOpts?.windows) ? pluginOpts.windows : [];
        chart.$compositeNarrativeOverlayRegions = [];
        if (!windows.length) return;
        const {ctx, chartArea, scales} = chart;
        const labels = Array.isArray(chart.data?.labels) ? chart.data.labels.map(v => String(v || '').trim()) : [];
        if (!ctx || !chartArea || !scales?.x || !scales?.y || !labels.length) return;
        const xScale = scales.x;
        const style = pluginOpts?.style === 'mid-band' ? 'mid-band' : 'top-band';
        const laneHeight = 16;
        const laneGap = 4;
        const reservedHeight = Number(pluginOpts?.reservedHeight || 0) || 0;
        const fontFamily = getComputedStyle(document.body).fontFamily || 'sans-serif';
        const getLeftEdge = (idx) => {
          if (idx <= 0) return chartArea.left;
          return (xScale.getPixelForValue(idx - 1) + xScale.getPixelForValue(idx)) / 2;
        };
        const getRightEdge = (idx) => {
          if (idx >= labels.length - 1) return chartArea.right;
          return (xScale.getPixelForValue(idx) + xScale.getPixelForValue(idx + 1)) / 2;
        };
        ...
        windows.forEach(window => {
          const startIdx = labels.indexOf(String(window.start_date || '').trim());
          const endIdx = labels.indexOf(String(window.end_date || '').trim());
          if (startIdx === -1 || endIdx === -1 || endIdx < startIdx) return;
          const boxLeft = Math.max(chartArea.left + 2, getLeftEdge(startIdx) + 3);
          const boxRight = Math.min(chartArea.right - 2, getRightEdge(endIdx) - 3);
          const boxWidth = boxRight - boxLeft;
          if (boxWidth < 12) return;

          const lane = Number.isFinite(window.lane) ? window.lane : 0;
          const boxYBase = Math.max(xScale.bottom + 4, chart.height - reservedHeight + 4);
          const boxY = Math.min(chart.height - laneHeight - 8, boxYBase + lane * (laneHeight + laneGap));
          const isActive = !!window.active_on_end_date;
          const isNonCrisis = !!window.is_non_crisis;
          const borderColor = isNonCrisis ? 'rgba(210,221,224,0.8)' : (isActive ? '#ff6b6b' : 'rgba(255,107,107,0.72)');
          const fillColor = isNonCrisis ? 'rgba(170,188,191,0.18)' : (isActive ? 'rgba(255,107,107,0.24)' : 'rgba(255,107,107,0.12)');
          const textColor = isActive ? '#fff1f1' : 'rgba(255,241,241,0.86)';

          drawRoundedRect(ctx, boxLeft, boxY, boxWidth, laneHeight, 7);
          ...
          const fitted = fitCompositeOverlayLabel(ctx, window.display_tag || window.tag || '', boxWidth - 12);
          if (fitted){
            if (style === 'mid-band'){
              const chipWidth = Math.min(boxWidth - 8, Math.max(54, ctx.measureText(fitted).width + 16));
              const chipX = boxLeft + ((boxWidth - chipWidth) / 2);
              drawRoundedRect(ctx, chipX, boxY + 3, chipWidth, laneHeight - 6, 5);
              ctx.fillStyle = isNonCrisis ? 'rgba(120,139,144,0.88)' : 'rgba(130,24,24,0.74)';
              ctx.fill();
            }
            ctx.fillStyle = textColor;
            ctx.fillText(fitted, boxLeft + (boxWidth / 2), boxY + (laneHeight / 2) + 0.5);
          }
          chart.$compositeNarrativeOverlayRegions.push({
            left: boxLeft,
            right: boxRight,
            top: boxY,
            bottom: boxY + laneHeight,
            window
          });
        });
        ctx.restore();
      ...
      afterEvent(chart, args){
        const hit = getCompositeNarrativeOverlayHit(chart, args?.event);
        if (chart?.canvas) {
          chart.canvas.style.cursor = hit ? 'pointer' : '';
          chart.canvas.title = hit
            ? `${String(hit.window?.display_tag || hit.window?.tag || '').trim()}`
            : '';
        }
        const eventType = String(args?.event?.type || '').trim().toLowerCase();
        if (hit && eventType === 'click' && selectedCompany) {
          const modalDate = String(hit.window?.end_date || '').trim();
          showNarrativeTimeline(
            { company: selectedCompany },
            isISODate(modalDate) ? modalDate : null
          );
        }
      }
    };
    ...
    if (compositeCtx) {
      negativeCompositeChart = new Chart(compositeCtx, {
        ...
        options: {
          ...
          layout: {
            padding: {
              ...commonOpts.layout.padding,
              bottom: (commonOpts.layout?.padding?.bottom || 0) + compositeNarrativeReservedHeight
            }
          },
          scales: {
            ...commonOpts.scales,
          ...
          onClick: (event, _elements, chart) => {
            if (getCompositeNarrativeOverlayHit(chart, event)) return;
          ...
          plugins: {
            ...commonOpts.plugins,
            compositeNarrativeOverlay: {
              windows: compositeNarrativeWindows,
              style: COMPOSITE_TAG_OVERLAY_STYLE,
              reservedHeight: compositeNarrativeReservedHeight
            },
        ...
        plugins: [splitFillPlugin, compositeNarrativeOverlayPlugin]
      });
    }
  ...
  function setCompanyModalState(company, activeTab){
    modalEntityState = {
      type: 'company',
  ...
  function renderModalTabs(){
    if (!modalEntityState || modalEntityState.type !== 'company' || !modalEntityState.company) return '';
    const tabs = [
  ...
  function bindModalTabs(){
    if (!modalEntityState || modalEntityState.type !== 'company' || !modalEntityState.company) return;
    const company = modalEntityState.company;
    modalContent.querySelectorAll('[data-modal-tab]').forEach(btn=>{
      btn.onclick = ()=>{
        ...
        if (tab === 'headlines') return void showHeadlines({ company }, d);
        if (tab === 'serp') return void showSerp({ company }, d);
        if (tab === 'features') return void showSerpFeatures({ company }, d);
      };
    });
  }
  ...
  function openModal(title,html,opts=null){
    ...
    const toolbarClass = (!tabsHtml && datePanel) ? 'modal-toolbar modal-toolbar-right' : 'modal-toolbar';
    const toolbarHtml = (tabsHtml || datePanel) ? `<div class="${toolbarClass}">${tabsHtml || ''}${datePanel || ''}</div>` : '';
  ...
  function bindInternalSummaryButton(buttonId, outputId, entityType, entityName, dateValue, featureType){
    const btn = modalContent.querySelector(`#${buttonId}`);
    ...
    btn.onclick = async () => {
      ...
      try{
        const data = await fetchCsv(`/api/internal/serp_feature_summary?date=${dateValue}&entity=${entityType}&entity_name=${encodeURIComponent(entityName)}&feature_type=${encodeURIComponent(featureType)}`);
        const text = (data && data.summary) ? data.summary : '';
        output.textContent = text || 'No summary available.';
  ...
  async function showHeadlines({company}, modalDate=null){
    setCompanyModalState(company, 'headlines');
    const d = String(modalDate || getModalDate()).trim();
    syncModalDateOverride(d);
    const modalOpts = { contextDate: d, onDateChange: (nextDate) => showHeadlines({ company }, nextDate) };
    openModal(`Headlines — ${esc(company)}`, `<div class="muted">Loading…</div>`, modalOpts);
    await _nextFrame();
    ...
    try{
      ...
      openModal(`Headlines — ${esc(company)}`, `<div class="muted">Could not load headlines for ${d}.</div>`, modalOpts); return;
    }
    let list=rows.filter(r=>String(r.company||'').trim()===company);
    if (!list.length){ openModal(`Headlines — ${esc(company)}`, `<div class="muted">No headlines for ${esc(company)} on ${d}.</div>`, modalOpts); return; }
      ...
      const insights = `
        ...
        <div class="serp-card serp-feature-summary">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
            ...
            <button id="headlineSummaryBtn" class="ghost-btn">Generate Summary</button>
          </div>
          <div id="headlineSummaryText" class="serp-snippet muted">Summary not generated.</div>
        </div>
      `;
      openModal(`Headlines — ${esc(company)}`, `${totalLabel}${insights}<div class="serp-cards">${cards}</div>${moreBtn}`, modalOpts);
      renderModalInsightDonuts('headlineSentChart', 'headlineControlChart', stats);
      bindInternalSummaryButton('headlineSummaryBtn', 'headlineSummaryText', 'brand', company, d, 'top_stories_items');
  ...
  async function showSerp({company}, modalDate=null){
    setCompanyModalState(company, 'serp');
    const d = String(modalDate || getModalDate()).trim();
    syncModalDateOverride(d);
    const modalOpts = { contextDate: d, onDateChange: (nextDate) => showSerp({ company }, nextDate) };
    openModal(`SERP — ${esc(company)}`, `<div class="muted">Loading…</div>`, modalOpts);
    await _nextFrame();
    const cacheKey = `${d}|${company}`;
    let rows=[];
    let total=null;
    ...
    catch{ openModal(`SERP — ${esc(company)}`, `<div class="muted">Could not load SERP file for ${d}.</div>`, modalOpts); return; }
    if (!rows.length){ openModal(`SERP — ${esc(company)}`, `<div class="muted">No SERP data found for ${d}.</div>`, modalOpts); return; }
    ...
    if (!matches.length){ openModal(`SERP — ${esc(company)}`, `<div class="muted">No SERPs for ${esc(company)} on ${d}.</div>`, modalOpts); return; }
    ...
    const renderModal = () => {
      ...
      const insights = `
        ...
        <div class="serp-card serp-feature-summary">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
            <h3 style="margin:0">AI Summary</h3>
            <button id="serpSummaryBtn" class="ghost-btn">Generate Summary</button>
          </div>
          <div id="serpSummaryText" class="serp-snippet muted">Summary not generated.</div>
        </div>
      `;
      openModal(`SERP — ${esc(company)}`, `${metrics}${totalLabel}${insights}<div class="serp-cards">${cards}</div>${moreBtn}`, modalOpts);
      renderModalInsightDonuts('serpSentChart', 'serpControlChart', stats);
      bindInternalSummaryButton('serpSummaryBtn', 'serpSummaryText', 'brand', company, d, 'organic');
      const btn = modalContent.querySelector('#modalLoadMore');
      if (btn) {
        btn.onclick = async () => {
          btn.disabled = true;
  ...
  async function showSerpFeatures({company}, modalDate=null){
    setCompanyModalState(company, 'features');
    const d = String(modalDate || getModalDate()).trim();
    syncModalDateOverride(d);
    const modalOpts = { contextDate: d, onDateChange: (nextDate) => showSerpFeatures({ company }, nextDate) };
    openModal(`SERP Features — ${esc(company)}`, `<div class="muted">Loading…</div>`, modalOpts);
    await _nextFrame();
    ...
    openModal(`SERP Features — ${esc(company)}`, `
      ...
      <div id="serpFeatureSummary" class="serp-card serp-feature-summary">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
          <h3 style="margin:0">AI Summary</h3>
          <button id="serpFeatureSummaryBtn" class="ghost-btn">Generate Summary</button>
        </div>
        <div class="serp-snippet muted">Summary not generated.</div>
      </div>
    ...
    async function renderFeature(reset = false){
      ...
      const cards = rows.map(r=>{
        ...
        const narrativeTag = String(r.narrative_primary_tag || '').trim();
        const narrativeGroup = String(r.narrative_primary_group || '').trim().toLowerCase();
        const narrativeDisplay = narrativeTag
          ? (narrativeGroup === 'non_crisis' ? `${narrativeTag} (non-crisis)` : narrativeTag)
          : '';
        const narrativeFlag = narrativeDisplay ? `<div class="serp-feature-flag">${esc(narrativeDisplay)}</div>` : '';
    ...
    async function renderFeatureSummary(){
      ...
      if (summaryBtn) {
        ...
        summaryBtn.onclick = async () => {
          ...
          if (snippetEl) snippetEl.textContent = 'Generating summary…';
          try{
  ...
  function formatDayDuration(days){
    const n = Number(days || 0);
    if (!Number.isFinite(n) || n <= 0) return '0 days';
    return `${n} day${n === 1 ? '' : 's'}`;
  }

  async function showNarrativeTimeline({company}, modalDate = null){
    const entityName = String(company || '').trim();
    if (!entityName) return;
    modalEntityState = null;
    const d = String(modalDate || getModalDate()).trim();
    syncModalDateOverride(d);
    const modalOpts = { contextDate: d, onDateChange: (nextDate) => showNarrativeTimeline({ company: entityName }, nextDate) };
    openModal(`Narrative Tags — ${esc(entityName)}`, `<div class="muted">Loading…</div>`, modalOpts);

    let payload = null;
    try{
      payload = await fetchCsv(NARRATIVE_TIMELINE_PATH(d, entityName));
    }catch{
      openModal(`Narrative Tags — ${esc(entityName)}`, `<div class="muted">Could not load narrative tags for ${d}.</div>`, modalOpts);
      return;
    }

    const tags = Array.isArray(payload?.tags) ? payload.tags : [];
    const lookbackDays = Number(payload?.lookback_days || NARRATIVE_TIMELINE_DAYS) || NARRATIVE_TIMELINE_DAYS;
    const activeCount = tags.filter(t => !!t.active_on_date).length;
    if (!tags.length){
      openModal(`Narrative Tags — ${esc(entityName)}`, `
        <div class="narrative-meta">
          <span>Lookback: ${lookbackDays} days</span>
          <span>Active tags on ${esc(d)}: 0</span>
        </div>
        <div class="muted">No narrative tags found for negative, non-financial Top Stories URLs on ${esc(d)}.</div>
      `, modalOpts);
      return;
    }

    const rowsHtml = tags.map(tag => {
      const active = !!tag.active_on_date;
      const isNonCrisis = !!tag.is_non_crisis;
      const isCrisis = !!tag.is_crisis && !isNonCrisis;
      const pillClass = isNonCrisis ? 'low' : (isCrisis ? 'high' : 'med');
      const label = String(tag.display_tag || tag.tag || '').trim();
      const startDate = String(tag.current_start_date || '').trim();
      const endDate = String(tag.current_end_date || '').trim();
      const currentDuration = active ? formatDayDuration(tag.current_duration_days) : '';
      const lastSeen = tag.last_seen_date ? esc(tag.last_seen_date) : '<span class="muted">—</span>';
      const mentionsToday = Number(tag.mentions_on_date || 0);
      const currentWindow = (active && startDate && endDate)
        ? `<div class="narrative-window"><span>${esc(currentDuration)}</span><span class="narrative-range">${esc(startDate)} → ${esc(endDate)}</span></div>`
        : '<span class="muted">—</span>';
      return `<tr class="${active ? 'active' : ''}">
        <td><span class="pill ${pillClass} narrative-pill">${esc(label || 'Unknown')}</span></td>
        <td>${currentWindow}</td>
        <td>${lastSeen}</td>
        <td>${mentionsToday}</td>
      </tr>`;
    ...
    openModal(`Narrative Tags — ${esc(entityName)}`, `
      <div class="narrative-meta">
        <span>Lookback: ${lookbackDays} days</span>
        <span>Active tags on ${esc(d)}: ${activeCount}</span>
      </div>
      <div id="narrativeSummary" class="serp-card serp-feature-summary">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
          <h3 style="margin:0">AI Summary</h3>
          <button id="narrativeSummaryBtn" class="ghost-btn">Generate Summary</button>
        </div>
        <div id="narrativeSummaryText" class="serp-snippet muted">Summary not generated.</div>
      </div>
      <div class="narrative-table-wrap">
    ...
    const summaryBtn = modalContent.querySelector('#narrativeSummaryBtn');
    const summaryText = modalContent.querySelector('#narrativeSummaryText');
    if (summaryBtn && summaryText){
      ...
      summaryBtn.onclick = async () => {
        ...
        try{
          const data = await fetchCsv(`/api/internal/serp_feature_summary?date=${d}&entity=brand&entity_name=${encodeURIComponent(entityName)}&feature_type=${encodeURIComponent('top_stories_items')}`);
          const text = String(data?.summary || '').trim();
          summaryText.textContent = text || 'No summary available.';
          summaryBtn.textContent = 'Regenerate Summary';
          summaryBtn.disabled = false;
        }catch{
