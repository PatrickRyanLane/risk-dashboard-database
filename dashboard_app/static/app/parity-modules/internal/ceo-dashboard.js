...
function runInlineScript(scope) {
  ...
  const SERP_FEATURE_SERIES_PATH = (ceo, feature, days = 30) =>
    `/api/v1/serp_feature_series?entity=ceo&entity_name=${encodeURIComponent(ceo)}&feature_type=${encodeURIComponent(feature)}&days=${days}`;
  const NARRATIVE_TAGS_PATH = d => `/api/v1/narrative_tags?entity=ceo&date=${encodeURIComponent(d)}`;
  const NARRATIVE_TIMELINE_DAYS = 90;
  const NARRATIVE_TIMELINE_PATH = (d, ceo, days = NARRATIVE_TIMELINE_DAYS) =>
    `/api/v1/narrative_timeline?entity=ceo&date=${encodeURIComponent(d)}&entity_name=${encodeURIComponent(ceo)}&days=${days}`;
  const NARRATIVE_OVERLAY_LIMIT = 0;
  const COMPOSITE_TAG_OVERLAY_STYLE = 'top-band';
  const NARRATIVE_OVERLAY_PATH = (ceo, startDate, endDate, limit = NARRATIVE_OVERLAY_LIMIT) =>
    `/api/v1/narrative_overlay?entity=ceo&entity_name=${encodeURIComponent(ceo)}&start_date=${encodeURIComponent(startDate)}&end_date=${encodeURIComponent(endDate)}&limit=${limit}`;
  ...
  async function selectCeo(name){
    ...
    await prefetchCompositeNarrativeOverlay(selectedCeo, getVisibleCompositeDateWindow());
    renderTable();
  ...
  function hookDatePager(allDates){
    const goPrev = async () => {
      ...
      if (selectedCeo){
        await prefetchCompositeNarrativeOverlay(selectedCeo, sliceWindow(allDates, dateWindowStart, DATE_WINDOW_SIZE));
      }
      renderCharts();
    };
    const goNext = async () => {
    ...
    try{
      ...
      rows.forEach(r => {
        ...
        if (ceo && company){
          ...
          if (fav || favCompany) favoriteCeos.add(ceo);
        }
  ...
  async function getNarrativesForDate(d){
    ...
    try{
      ...
      (rows || []).forEach(r=>{
        const ceo = String(r.entity_name || r.ceo || '').trim();
        if (!ceo) return;
        const key = normEntityKey(ceo);
        if (!key) return;
  ...
  async function buildRowsForDate(d){
    ...
    for (const [ceo, a] of articles.entries()){
      ...
      const narrative = narratives.get(normEntityKey(ceo)) || null;
  ...
  function renderTable(){
    ...
    if (currentSort.key){
      rows.sort((a,b)=>{
        ...
        if (typeof ka === 'string') {
          return currentSort.dir * ka.localeCompare(String(kb ?? ''));
        }
        if (typeof kb === 'string') {
          return currentSort.dir * String(ka ?? '').localeCompare(kb);
        }
    ...
    const rowsHTML = pageRows.map(r=>{
      ...
      const favStar = `<button class="fav-inline ${favActive ? 'active' : ''}" data-ceo="${esc(r.ceo)}" data-company="${esc(r.company)}" title="Toggle favorite">★</button>`;
      ...
      const narrativeHtml = `<button type="button" class="pill narrative-pill-btn ${narrativeButtonClass}" data-action="narrative-timeline" data-ceo="${esc(r.ceo)}" title="${esc(narrativeButtonTitle)}">${esc(narrativeButtonLabel)}</button>`;
    ...
    tbody.querySelectorAll('.fav-inline[data-ceo]').forEach(btn=>{
      btn.onclick = async (e)=>{
        e.stopPropagation();
        const ceo = btn.dataset.ceo || '';
        const company = btn.dataset.company || '';
        if (!ceo) return;
        const next = !favoriteCeos.has(ceo);
        btn.disabled = true;
        const ok = await updateFavorite('ceo', ceo, next, company);
        if (ok) {
          if (next) favoriteCeos.add(ceo);
          else favoriteCeos.delete(ceo);
    ...
    tbody.querySelectorAll('button[data-action="narrative-timeline"]').forEach(btn=>{
      btn.onclick = (e)=>{
        e.stopPropagation();
        const ceo = String(btn.getAttribute('data-ceo') || '').trim();
        if (!ceo) return;
        showNarrativeTimeline({ ceo });
      };
    });
  ...
  function renderCharts(){
    ...
    const compositeNarrativeWindows = getCompositeNarrativeOverlay(selectedCeo, d);
    ...
    const compositeNarrativeOverlayPlugin = {
      ...
      afterEvent(chart, args){
        ...
        if (hit && eventType === 'click' && selectedCeo) {
  ...
  async function showHeadlines({ ceo, company }, modalDate = null) {
    ...
    const renderCards = (items) => items.map(r => {
      ...
      return `
        <div class="serp-card" data-mention-id="${esc(mentionId)}" data-sentiment-override="${esc(r.sentiment_override||'')}" data-control-override="${esc(r.control_override||'')}" data-llm-label="${esc(r.llm_label||'')}" data-current-sentiment="${esc(s)}" data-current-control="${esc(ctrl)}">
    ...
    const renderModal = () => {
      ...
      bindInternalSummaryButton('headlineSummaryBtn', 'headlineSummaryText', 'ceo', ceo, d, 'top_stories_items');
  ...
  async function showSerp(row, modalDate = null){
    ...
    const renderModal = () => {
      ...
      bindInternalSummaryButton('serpSummaryBtn', 'serpSummaryText', 'ceo', row.ceo, d, 'organic');
      const btn = modalContent.querySelector('#modalLoadMore');
      if (btn) {
        btn.onclick = async () => {
          btn.disabled = true;
  ...
  async function showSerpFeatures({ceo, company}, modalDate = null){
    ...
    async function renderFeatureSeries(){
      ...
      let series = [];
      try{
        series = await fetchCsv(SERP_FEATURE_SERIES_PATH(ceo, feature, FEATURE_MODAL_DAYS));
      }catch{
    ...
    async function renderFeatureSummary(){
      ...
      if (summaryBtn) {
        ...
        summaryBtn.onclick = async () => {
          ...
          try{
            const data = await fetchCsv(`/api/internal/serp_feature_summary?date=${d}&entity=ceo&entity_name=${encodeURIComponent(ceo)}&feature_type=${encodeURIComponent(feature)}`);
            const text = (data && data.summary) ? data.summary : '';
  ...
  async function showNarrativeTimeline({ceo}, modalDate = null){
    const ceoName = String(ceo || '').trim();
    if (!ceoName) return;
    modalEntityState = null;
    const d = String(modalDate || getModalDate()).trim();
    syncModalDateOverride(d);
    const modalOpts = { contextDate: d, onDateChange: (nextDate) => showNarrativeTimeline({ ceo: ceoName }, nextDate) };
    openModal(`Narrative Tags — ${esc(ceoName)}`, `<div class="muted">Loading…</div>`, modalOpts);

    let payload = null;
    try{
      payload = await fetchCsv(NARRATIVE_TIMELINE_PATH(d, ceoName));
    }catch{
      openModal(`Narrative Tags — ${esc(ceoName)}`, `<div class="muted">Could not load narrative tags for ${d}.</div>`, modalOpts);
      return;
    }
    ...
    if (!tags.length){
      openModal(`Narrative Tags — ${esc(ceoName)}`, `
    ...
    if (summaryBtn && summaryText){
      ...
      summaryBtn.onclick = async () => {
        ...
        try{
          const data = await fetchCsv(`/api/internal/serp_feature_summary?date=${d}&entity=ceo&entity_name=${encodeURIComponent(ceoName)}&feature_type=${encodeURIComponent('top_stories_items')}`);
